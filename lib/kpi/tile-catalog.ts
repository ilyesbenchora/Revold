...
const BILLING_TILES: TileSuggestion[] = [
  ...
  // ── Factures FOURNISSEURS (reçues — Pennylane & co) : toujours séparées des
  //    factures clients ; le solde net combine les deux sens. ──
  { id: "fournisseurs_facture", label: "Factures fournisseurs (montant)", description: "Montant total des factures fournisseurs reçues", unit: "currency", sourceCategory: "billing", aggSpec: { entity: "supplier_invoices", groupBy: "status", measure: "sum", field: "amount_total" } },
  { id: "fournisseurs_a_payer", label: "Reste à payer fournisseurs", description: "Dettes fournisseurs : reste à payer sur les factures reçues non soldées", unit: "currency", sourceCategory: "billing", aggSpec: { entity: "supplier_invoices", groupBy: "status", measure: "sum", field: "amount_due" } },
  { id: "fournisseurs_payees", label: "Payé aux fournisseurs", description: "Montant réellement payé sur les factures fournisseurs", unit: "currency", sourceCategory: "billing", aggSpec: { entity: "supplier_invoices", groupBy: "status", measure: "sum", field: "amount_paid" } },
  { id: "factures_fournisseurs", label: "Factures fournisseurs (nombre)", description: "Nombre de factures fournisseurs reçues", unit: "count", sourceCategory: "billing", aggSpec: { entity: "supplier_invoices", groupBy: "status", measure: "count" } },
  { id: "solde_facturation", label: "Solde facturé (clients − fournisseurs)", description: "Montant facturé aux clients moins les factures fournisseurs reçues", unit: "currency", sourceCategory: "billing", aggSpec: { entity: "invoices", groupBy: "direction", measure: "sum", field: "net_total" } },
  { id: "solde_cash_factures", label: "Solde encaissé − payé fournisseurs", description: "Encaissé sur les factures clients moins payé sur les factures fournisseurs", unit: "currency", sourceCategory: "billing", aggSpec: { entity: "invoices", groupBy: "direction", measure: "sum", field: "net_paid" } },
