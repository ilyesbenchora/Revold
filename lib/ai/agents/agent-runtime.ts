...
export async function runAgentTurn(opts: {
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    for (const block of res.content) {
      ...
      // Proposition de graphique → capturée ; l'UTILISATEUR choisit le format
      // parmi les options, puis enregistre via le CTA.
      if (block.name === PROPOSE_CHART_TOOL) {
        ...
        results.push({
          ...
          content: chartProposal
            ? "Graphique proposé à l'utilisateur : il choisira le format d'affichage parmi les options, ajustera la période, puis l'enregistrera via le CTA. Réponds au fond de sa question (lecture, diagnostic, recommandation) en une phrase ou deux. Ensuite SEULEMENT, et uniquement si un suivi chiffré dans le temps est pertinent, tu PEUX proposer une alerte de suivi via propose_action. Sinon, conclus sans alerte."
