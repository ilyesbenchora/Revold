...
| B5 | Enrichissement : source LinkedIn pour le nombre d'employés (optimiser le taux) ; bouton « Enrichir mon CRM » sur le bloc ; à la fin, tableau des entreprises enrichies (même visualisation que le bloc du dessous) | [x] (LinkedIn en beta, preference stockee) |
...
|---|---|---|---|
| 2026-09-12 | Blog : publication programmée | `app/blog/published.ts` (un article dont la date est future est invisible partout : liste, page, RSS, sitemap, llms.txt ; ISR 1 h), `articles-2026-09.ts` = calendrier éditorial 2 articles/semaine (mardi + jeudi) : 2 publiés le 12/09 (KPI commerciaux, bon taux de churn) + 8 programmés jusqu'au 08/10 (pipeline, deals stagnants, prévision des ventes, date de fermeture HubSpot, MRR/ARR, fuite de revenus cas concrets, SIREN/API Sirene, NRR) ; chaque article = réponse directe + liens internes + FAQ (FAQPage) + mots-clés ; liste du blog en composant serveur avec métadonnées | Ajouter un article = une entrée datée dans `editorialArticles`, rien d'autre |
| 2026-09-12 | SEO / GEO vague 2 | Analyse de marché (SERP FR) → 8 guides longue traîne à fort potentiel (fuite de revenus, KPI RevOps, RevOps vs Sales Ops, tableau de bord commercial, HubSpot × Pennylane / Stripe / Sage, audit CRM HubSpot), 3 comparatifs (Forecastio, outil Prévisions HubSpot, Excel), glossaire 41 termes (DefinedTermSet), 4 calculateurs gratuits (/outils, WebApplication), menu « Ressources » dans la navbar, encart auteur sur le blog ; sitemap 67 URLs | Rapport d'analyse publié en artefact ; reste hors code : Search Console + Bing, fiches d'entité (G2, Capterra, Crunchbase, LinkedIn), backlinks agences (Cartelis, Ceres) |
| 2026-09-11 | SEO / GEO | Socle `lib/seo` (faits de marque + JSON-LD Organization/WebSite/SoftwareApplication/FAQPage/Breadcrumb/Person/BlogPosting), 10 guides mots-clés à la racine (plateforme RevOps, pilotage RevOps, pilotage performance d'entreprise, plateforme Revenue Intelligence, plateforme revenue, logiciel RevOps, revenue intelligence, forecast commercial, tableau de bord RevOps, réconciliation CRM facturation), 9 pages « Alternative à X » (Clari, Grow, Looker Studio, Gong, Power BI, Salesforce RI, Aviso, BoostUp, Metabase) + hub /comparatif, /a-propos, /llms.txt + /llms-full.txt, robots.txt ouvert aux robots IA, sitemap 50 URLs, /tarifs et /pourquoi-revold réindexés, colonne Ressources du footer | Ajouter une requête = une entrée `KEYWORD_PAGES` ; un concurrent = une entrée `COMPETITORS`. Reste hors code : soumission Search Console + Bing Webmaster, backlinks |
| 2026-09-09 | Tour de contrôle | Brief personnalisé par équipe (Paramètres → Tour de contrôle) : équipe (admin libre / membre verrouillé sur son pôle), pipelines, suggestions Ventes/Marketing/Service client/Comptabilité avec périodes multi-cases (semaine, mois, mois prochain, trimestre, trimestre suivant, année), seuil de stagnation, propriété de date remplaçant closedate sur les prévisions, propriétés CRM vérifiées (existence + type + rapprochement) puis suggestions personnalisées calculées sur les vraies données avant validation | `lib/voice/brief-team.ts` (catalogue), `brief-team-engine.ts` (moteur déterministe, repli HubSpot direct tant que la propriété n'est pas synchronisée), routes `/api/voice/brief-team/{options,verify}`, section `team` du digest, ETL embarque les propriétés du brief ; texte du brief effacé en fin de lecture |
| 2026-09-08 | Hiérarchie comptes | Recherche vue Table (en masse) + NOUVELLE sous-page « Contacts sans entreprise » (association bulk que HubSpot n'a pas) | (1) Console hiérarchie : barre de recherche vue Table — filtre noms parent/enfant, domaine, SIREN/SIRET ; pagination et « tout sélectionner » sur la file filtrée. (2) `/dashboard/hierarchie/contacts-sans-entreprise` : contacts orphelins (company_id null + hubspot_id présent) — SUGGESTIONS AUTO par domaine email ↔ companies.domain (domaines grand public exclus, domaine ambigu = 1 seule suggestion) validées par groupe, + mode MANUEL (recherche contact, sélection en masse, recherche entreprise cible `/api/orphan-contacts/companies`). POST `/api/orphan-contacts` : écrit d'abord HubSpot (associations v4 batch/create, types 279 + 1 primary, repli 279 seul, chunks 100, stop sur 403 scope) puis répercute `contacts.company_id` en canonique + journal sync_logs (outbound contact_company_association). Aucune écriture sans clic (opt-in). Lien depuis l'en-tête Hiérarchie. tsc + eslint + 95 tests OK. |
| 2026-09-03 | Sources par page (suite) | Fix page Facturation vide malgré Pennylane mappé : la source ne se choisit plus aveuglément | `fetchPaiementFacturationFor` prenait `mappedKeys[0]` — avec un mapping [hubspot, pennylane], HubSpot (sans module Invoices) répondait vide et la page affichait « Aucun outil de facturation connecté » alors que Pennylane avait les données. Refactor : `fetchForSource(sourceKey)` extrait, et sans override du switcher on essaie les outils mappés dans l'ordre FACTURATION d'abord (non-hubspot avant hubspot) en servant le premier avec `hasData` ; l'override du switcher reste strict (l'outil cliqué fait foi, sans repli). Touche les 3 pages de la section (vue d'ensemble, facturation, paiement). tsc + eslint + 95 tests OK. |
| 2026-08-27 | Sources par page | Fix « connecte un outil » alors que Pennylane est connecté : fallback outils connectés sans mapping | Deux causes : l'état neutre des pages Données exigeait un mapping « Outil source par page », et le flux de connexion par CLÉ API (Pennylane, Stripe…) n'appelait jamais `autoMapSingleTool` (seul le callback OAuth le faisait) → jamais de mapping créé. Fix : (1) Trésorerie/facturation — sans URL ni mapping, tous les outils connectés pertinents sont présélectionnés, invite seulement si rien de connecté (texte → Bibliothèque) ; (2) `PageSourcesGate` — même fallback par `categories` de la page (Appels, Service client, Données, Performances, Publicité), `PageSourcesFooter` prend les mêmes catégories pour lister les sources effectives (9 pages mises à jour) ; (3) `connectToolAction` appelle `autoMapSingleTool` comme le flux OAuth. Un mapping explicite garde toujours la priorité (ISO Paramètres). tsc + eslint + 95 tests OK. |
| 2026-08-24 | Prévisionnel trésorerie | Fix « lignes droites » de la Projection 12 mois : activité récurrente saisonnalisée dans le moteur | Cause : le rendu (recharts `monotone`, vérifié partout — ForecastChart, TresoLineChart, ReportChart agents : tous en courbes) n'était PAS en faute ; le modèle ne projetait AUCUN encaissement après ~2 mois (factures ouvertes et deals tombent sur les 1ers mois) + charges médiane constante → pente constante = droite. Fix `treasury-forecast.ts` : encaissements récurrents saisonnalisés = moyenne (médiane des encaissements réels hors mois courant, même mois an −1 via `monthlyFlows.month` ajouté à CashflowData) appliqués en COMPLÉMENT (max(0, base − factures du mois) — pas de double compte) ; charges saisonnalisées pareil, en complément des fournisseurs + fiscal connus (corrige aussi un double compte existant). Scénarios resémantisés : Prudent = récurrent ×0,8 sans pipeline · Probable = récurrent + pipeline pondéré · Ambitieux = récurrent + pipeline plein. Nouveau champ `encaissementsRecurrents` exposé (colonne « Récurrent » du détail mensuel, footnote), légendes mises à jour (ForecastChart + 2 pages). tsc + eslint + 95 tests OK. |
| 2026-08-24 | ROI Trésorerie | Relances d'impayés par EMAIL avec séquence auto + CTA vert « cash récupéré » | Bloc « Relances & cash récupéré » : « Relancer » ouvre une modale — mail pré-rédigé côté serveur (n°, montant, échéance, lien PDF public Pennylane désormais capturé dans `source_metadata.invoice_url`), destinataire = contact de la facture (éditable), récurrence 0/7/14 j + plafond dur 2/3/5 envois. Valider = envoi RÉEL d'abord (Resend, reply-to = user) puis trace `invoice_reminders` (channel email, mécanique cash récupéré inchangée) + ligne `invoice_reminder_sequences` (migration 20260824000001, unique org×invoice, auto au build). Renvois par cron horaire `send-invoice-reminders` (45 * * * *, monitoré) : re-vérifie la facture AVANT envoi → payée = stop reason `paid` + attribution du récupéré sans attendre l'ouverture du bloc ; plafond → stop `max_reached` ; bouton « stopper » (DELETE). GET renvoie aussi les factures récupérées <30 j (elles sortent de la requête « en retard » une fois payées) → badge VERT « Cash récupéré le X · montant ». Ancien flux « marquer relancée sans email » conservé en lien discret dans la modale. `sendDirectEmail` exporté (send.ts). Icônes 💶/🎉 retirées du bloc. tsc + eslint + 95 tests OK. |
| 2026-08-23 | Connecteurs | Aircall + Qonto live (source + ping + connecteur sync) ; Sellsy, Meta Ads, Google Ads, LinkedIn Ads dé-grisés (comingSoon retiré) | **Aircall** (téléphonie, clé API api_id+api_token) : `sources/aircall.ts` (ping `/v1/ping`, appels 90 j paginés `meta.next_page_link`), connecteur → table `activities` (type "call", sujet/durée/date, contact via email resolveContact sinon matching téléphone — 9 derniers chiffres — sans création), dédup `source_links` entity_type `activity`, compteur `calls` (types + orchestrateur « Appels »), entrée identifier-catalog. **Qonto** (banque pro, NOUVEAU au catalogue billing : organization_slug + secret_key) : `sources/qonto.ts` (ping `/v2/organization`, transactions 12 mois par IBAN), connecteur → `payments` signés (crédit +, débit −, side/label/iban en source_metadata), resolveCompany sur la contrepartie des encaissements uniquement, entrée identifier-catalog. **Ads OAuth** : rien à coder (flow générique + fetchers déjà prêts) — LinkedIn Ads utilisable direct (env posées) ; Google Ads et Meta Ads afficheront `oauth_env_*` tant que `GOOGLE_OAUTH_CLIENT_ID/SECRET` et `META_APP_ID/SECRET` ne sont pas sur Vercel (redirect URIs documentées dans CLAUDE.md) ; `GOOGLE_ADS_DEVELOPER_TOKEN` requis pour les métriques Google Ads. tsc + eslint OK (fichiers touchés). |
| 2026-08-20 | Réconciliation « niveau clôture » | Lien deal ↔ factures + file d'apurement + avoirs & périodisation (3 chantiers, 3 commits) | (1) **Deal ↔ factures** : migration `invoices.deal_id` (+ deal_link_method) ; moteur de matching (candidates même entreprise, fenêtre close −30 j/+365 j ; exact ±1 % = high, somme = combo, sinon manuel), API deal-invoices (GET/POST lier/DELETE délier — jamais d'écriture sans confirmation), bloc Trésorerie « Rapprochement deal ↔ factures » (propositions + table écart par deal soldé/partiel/sur-facturé/non facturé), recette `deal_billing_gap` alertable (couverture = deals rattachés/gagnés). (2) **File d'apurement** : table `recon_gap_reviews` (statut open/justified/to_fix/fixed + note, unique org×company), écarts par entreprise (tolérance 1 €/1 %) avec AVOIRS exposés (dont avoirs −X €), bloc à filtres + note inline + export CSV (BOM UTF-8). (3) **Périodisation** : `computePeriodizedGap` — signé (close_date) vs facturé (issued_at) par trimestre (6 derniers, axes continus, avoirs par trimestre), table dans le bloc d'apurement. Migrations auto. tsc + eslint + tests réconciliation OK. |
| 2026-08-19 | Onboarding « première valeur » | Tours guidés Tableaux de bord + funnel de câblage, wizard qui mène au premier tableau | Réutilise l'infra FeatureTour (coach marks localStorage, une fois par utilisateur) : (1) tour « boards » sur /tableaux-de-bord — composer (ancre board-create sur ＋ Nouveau tableau, rangée racine), interroger (board-ask), partager (board-share) ; (2) tour « data-funnel » à la première ouverture du funnel de création (ancres funnel-sources/funnel-kpis/funnel-custom, vignette z-200 au-dessus de la modale) ; (3) étape finale du wizard d'onboarding : CTA principal « Créer mon premier tableau → » vers /dashboard/templates (aperçus des templates), secondaire « Aller au dashboard » — complete(destination). |
| 2026-08-19 | Trois features inspirées de la veille concurrentielle (DataGOL) | Tableaux conversationnels + partage public lecture seule + dictionnaire de métriques | (1) **Tableaux conversationnels** : champ « Pose une question sur ce tableau » (BoardAsk, dictée incluse) sur Vue d'ensemble/tableaux/onglets → `/api/boards/ask` (runAgentTurn outillé aggregate_canonical seulement, contextualisé par la composition réelle du tableau, suivis 3 échanges). (2) **Partage public** : bouton Partager → lien `/partage/<uuid>` (table `board_shares`, RLS org, révocable — nouveau jeton à la recréation) ; page publique hors middleware, service role borné à l'org, tuiles (resolveAddedTiles) + tables (computeAggregate + période) recalculées en direct, DataPreview, noindex. (3) **Dictionnaire de métriques** : Paramètres → Métriques (table `metric_definitions`, CRUD `/api/metric-definitions`, 40 max) — label + définition maison + unité ; `metricDictionaryDirective` injectée dans le chat des agents, `/api/boards/ask` et le câblage des KPIs personnalisés (resolve-custom-kpi). Migrations 20260819000003/-04 auto-appliquées. tsc + eslint OK. |
| 2026-08-19 | Site marketing — à jour produit | Mentions « coach » retirées (13 emplacements) + enrichissement v2 mis en avant | Les coachs par métier n'existent plus (retirés du produit le 2026-08-15) : home (features, personas, espaces par pôle), produits/insights-ia (subtitle, cards, how-it-works, setups), 4 pages solutions, pourquoi-revold (desc + statLabel), tarifs, 4 pages équipes (« Coach X IA 24/7 » → « Agent X IA 24/7 »), navbar — remplacés par « agents experts par domaine, pilotables à la voix ». « Coaching data-driven » (piloter-performance) conservé : coaching humain des managers. Articles de blog conservés (contenu éditorial générique). Enrichissement v2 mis en avant : home (feature rapprochement) et produits/resolution-entites (subtitle, feature Sirene+INPI : effectifs, CA, secteur NAF, statut juridique, capital social, adresse siège ; écriture HubSpot champs vides + refresh 90 j + nouvelles entreprises en continu). |
| 2026-08-18 | Tour de contrôle — orbe verte | L'orbe passe au vert émeraude quand un contenu du brief est finalisé/exécuté/atteint | Signal branché sur les compteurs du digest (`/api/voice/digest`) : objectif atteint (`reachedObjectives`), action exécutée dans les outils (`actionsExecuted`), enrichissement terminé (nouveau flag `enrichmentCompleted` = plus rien en attente + base identifiée). État `achieved` mis à jour au chargement ET à chaque brief lu ; rendu canvas : halo + particules émeraude (même charte que l'anneau de santé) à la place du fuchsia, anneau émeraude avec halo, tooltip « Bonne nouvelle… demande ton brief ». Priorités préservées : écoute (ambre) et santé critique (rouge pulsé) passent devant. |
| 2026-08-18 | Enrichissement — tuile 90 j | Tuile « Rafraîchies < 90 j » supprimée | L'entretien est automatique et vérifié : cron `enrich-companies` toutes les 5 min (vercel.json), périmètre = nouvelles entreprises (identités jamais scannées) + rafraîchissement des faits > 90 j. Ce compteur interne de maintenance n'apportait rien (« données évolutives — à entretenir » laissait croire à une action manuelle) — la ligne de tuiles reste : Entreprises, couverture par champ actif, À valider. |
| 2026-08-18 | Enrichissement — détail dans l'Historique | Bloc « Entreprises enrichies » fusionné dans l'Historique + pagination serveur sur toute la base | Le bloc autonome (carte dépliable, 300 fiches max chargées serveur) est SUPPRIMÉ (`enriched-companies-table.tsx` supprimé) ; le détail vit désormais en dropdown DANS « Historique des enrichissements » (EnrichmentBackfillRunner). `EnrichedCompaniesPanel` refondu en client auto-alimenté : nouvelle API `/api/enrichment/enriched-companies` (pagination serveur page/pageSize, count exact) — TOUTES les fiches sont parcourables, plus de plafond aux 300 plus récentes. Périmètre élargi pour coller aux compteurs de l'historique : `siren non nul OU enriched_at non nul` (fiches touchées par le moteur), colonne « Statut juridique » ajoutée (repli si migration absente). tsc + eslint OK. |
| 2026-08-18 | Tuiles KPI — câblage & édition | Vérification du câblage obligatoire pour les tuiles suggérées + édition ✎ des tuiles ajoutées | (1) **Câblage** : une tuile issue d'une suggestion passe désormais par le MÊME écran « Vérification du câblage » que les KPIs personnalisés, câblé en déterministe sans agent (`verifyDraft` → proposal agent « Revold », preuve chiffrée via recompute) — CTA step 2 « Vérifier le câblage », plus de création directe ; `refreshVerif` affiche la valeur de la ligne cible (target non-percent : « Valeur de la ligne Gagnés ») ; sélecteur Pipeline de la Vérification étendu aux dims statut ; changer le regroupement purge target/percent. Exceptions : forecast_type et fiscal (valeur unique, création directe). (2) **Édition** : bouton ✎ sur chaque tuile AJOUTÉE en mode Personnaliser (kpi-tiles-editor, event `revold:edit-kpi-tile`) → funnel prérempli depuis l'agg_spec (entité, mesure, pipeline, target, période, sources) → Affichage → Vérification → PATCH `/api/page-tiles/[id]` (nouveau : title/agg_spec/unit_mode, reset prev_value pour l'évolution) ; tuiles forecast_type : renommage seul ; tuiles par défaut : masquage seulement (calcul en dur). `ResolvedAddedTile.aggSpec/forecastType` exposés jusqu'à l'éditeur. tsc + eslint OK. |
| 2026-08-18 | Enrichissement — jauge | La jauge repart dès qu'un nouveau champ est coché dans les Paramètres | (1) `/api/enrichment-settings` : à l'enregistrement, les champs NOUVELLEMENT cochés (prev false → true) remettent en file les fiches où ils manquent — `enriched_at → null` (fiches avec SIREN, colonne du champ null) et `sirene_checked_at → null` si SIREN vient d'être activé. Plus besoin d'attendre le clic « Enrichir mon CRM » : le statut montre le restant immédiatement et le robot (cron) traite. (2) `/api/enrichment/status` : jauge = fiches à jour / base totale ((total − remaining)/total) au lieu de processed/(processed+remaining) qui restait figée haut — cocher « Statut juridique » fait redescendre la jauge au vrai avancement du chantier. tsc + eslint OK. |
| 2026-08-18 | Cohortes — objet HubSpot | Sélecteur « Objet HubSpot » par cohorte + vérification à l'enregistrement ciblée sur cet objet | `CohortMapping.object` (contacts/companies/deals, "" = auto) : sélecteur en PREMIER champ de chaque cohorte, défauts standard (Sources → Contact, Secteur/Segment → Entreprise, Priorité → Deal), persisté (`cohort-mappings` route, whitelist). À l'enregistrement, la vérification HubSpot cible l'objet choisi (`checkHubSpotProperty`) au lieu du scan aveugle 3 objets ; nouveau `fallbackAny` sur `/api/settings/hubspot-properties` : introuvable sur l'objet choisi → recherche de repli sur les autres objets et l'objet du mapping est CORRIGÉ automatiquement (badge « ✓ DANS LE CRM · Contact »), comme le nom API retrouvé par libellé. Statut serveur (page) recalculé sur l'objet enregistré. Messages d'erreur précisent l'objet. tsc + eslint OK. |
| 2026-08-18 | KPIs Ventes par pipeline | 6 nouveaux presets KPI ciblés pipeline sur la page Ventes + 3 dims deals (status/outcome/close_date_state) | (1) **Moteur** (`tool-library.ts`) : colonnes deals + `pipeline_stages(is_closed_won,is_closed_lost)` ; nouvelles dims `status` (En cours/Gagnés/Perdus), `outcome` (deals clôturés seulement — percent_of_total sur « Perdus » = vrai taux de perte), `close_date_state` (deals en cours : Dépassée/À jour/Sans close date) avec fallback HubSpot (stageMap) comme stage/pipeline ; libellés exportés (`DEAL_STATUS_LABELS`/`CLOSE_DATE_LABELS`). (2) **Presets Ventes** (`data-table-presets.ts`) : `TablePreset.target`/`percentOfTotal` ; 6 tuiles « (par pipeline) » — montant pondéré des deals en cours (weighted, hubspot requis), montant des deals en cours, CA signé, montant des deals perdus, taux de perte (%), % de close dates dépassées ; `weighted_forecast_stage` relabellisé « Répartition du CA pondéré par étape (par pipeline) ». (3) **Funnel** (`page-data-tables.tsx`) : sélecteur Pipeline étendu de `stage` à `PIPELINE_SCOPED_DIMS` (stage/status/outcome/close_date_state) — le pipeline se choisit à l'étape Affichage et est embarqué dans l'agg_spec (+ rappelé sur la tuile) ; presets à `target` = déterministes (pas d'agent), tuile imposée (`tileOnly`), toggle « % d'une ligne » masqué, aperçu tuile calcule la ligne cible/le taux. (4) **Drill-down ciblé** : `TileDrill.bucket` — cliquer « CA signé » n'ouvre que les deals gagnés du pipeline. Tests : 4 nouveaux cas dans `compute-aggregate.test.ts` (14/14 OK), tsc + eslint OK. |
| 2026-08-13 | Routines de chat + form coaching | Onglet « 🕘 Routines » dans le chat des agents, rapports de routine dans « Rapports enregistrés » (badge), champs coaching obligatoires | (1) **Routines** : catalogue de suggestions par coach (`lib/ai/agents/routine-catalog.ts` — coach des ventes : récap ventes semaine/mois/semestre tous les jours à 9h00 ; marketing, finance, data adaptés + fallback générique) ; store localStorage `components/agents/routines.ts` (fréquence daily/weekly/monthly + heure, pause/réactivation, `isRoutineDue` ne déclenche pas l'occurrence antérieure à la création) ; onglet Routines dans `paiement-agent-chat.tsx` (`routines-tab.tsx` : suggestions à activer, routines actives éditables, routine personnalisée, « Exécuter maintenant »). Runner client (`useAgentRoutines`) : à l'ouverture de la page (puis chaque minute), routine échue → POST `/api/agents/[agentKey]` avec directive **rapport visuel qualité tables de données** (blocs kpi + graphique + table, `query` sur chaque bloc pour le recalcul déterministe) → `addSavedReport` avec `origin: "routine"`. (2) **Rapports enregistrés** : `SavedReport.origin`/`routineLabel`, badge « 🕘 Routine · … » + « Généré le » dans le carrousel sous le chat et dans Mes rapports. (3) **Formulaire coaching** (coach-agenda + coaching-agenda) : objectifs, pains et cadence **obligatoires** (astérisques, validation bloquante, messages) avec la date du RDV placée **sous** la cadence. Build prod + eslint (fichiers touchés) OK. |
| 2026-08-10 | Blocs configurables | Tuiles KPI + blocs retirables/ajoutables sur 6 pages (Ventes, Marketing, Trésorerie, Service client, Équipes, Audit qualité) | Nouvelle table `page_tiles` (kind `kpi`/`hide_tile`/`hide_block`, RLS org) + routes `/api/page-tiles` (GET/POST/DELETE). Tuiles : défauts en dur conservés (clé stable, masquables/réaffichables), ajout depuis les **suggestions du pôle** = catalogue `kpisByTeam` extrait du modal d'alerte vers `lib/alerts/kpi-catalog.ts` (partagé) + KPIs billing/support en `agg_spec` (`lib/kpi/tile-catalog.ts`) + KPI personnalisé via `/api/tracking/preview` ; valeurs résolues avec le **même contrat que le cron d'alertes** (`resolveKpiValue`/`valueFromAggSpec`) dans `lib/kpi/page-tiles.ts` + `ConfigurableKpiTiles`/`KpiTilesEditor`. Blocs : `RemovableBlock` (✕ au survol → hide_block) + `BlocksManager` (« ＋ Ajouter un bloc » : réafficher les masqués + presets `TABLE_PRESETS` filtrés par outils connectés, créés en 1 clic dans `page_data_tables` → event `revold:reload-page-tables`). Équipes + Audit qualité gagnent une section « Tables de données » (nouvelles clés `audit_adoption`/`audit_donnees` dans PAGE_LABELS/TABLE_PRESETS/PAGE_AGENT_KEY/PAGE_ALERT_TEAM). tsc + eslint (fichiers touchés) OK. **Manuel** : migration `20260810000001_page_tiles.sql`. |
| 2026-07-25 | Page Alignement | Automatisations → Alignement (collecte + recettes + page + agent) | (1) **Collecte** : ETL contacts + `hs_v2_date_entered_{lead,mql,sql,opportunity,customer}` → colonnes datées `contacts.{lifecycle_stage,hs_created_at,lead_date,mql_date,sql_date,opportunity_date,customer_date}` (migration `20260725000001`, backfill lifecycle_stage/hs_created_at depuis raw_data, repli sans colonnes si migration absente) ; vraie `createdate` HubSpot mappée sur `deals.created_date` (avant : CURRENT_DATE du 1er sync) ; `is_mql`/`is_sql` re-peuplés (sémantique « a atteint le stade ») ; fix `sales_cycle_days` (created_date au lieu de created_at). (2) **Recettes de délai** (moteur réconciliation, médiane + sample/denominator + gate couverture) : `mql_to_deal_created`, `deal_won_to_first_invoice`, `invoice_to_payment` — disponibles pour alertes/objectifs. (3) **Page** `/dashboard/process` réécrite : 4 tuiles (MQL→deal, cycle réel, deal→facture, DSO), bloc vélocité lifecycle (temps médian + taux de passage par étape), relais mkg→ventes (MQL abandonnés ≥14j liste nominative), ventes→finance (délais + fuite de revenu), finance→CS (MRR à risque) ; bloc workflows SUPPRIMÉ (endpoint pauvre) ; `lib/alignment/velocity.ts`. Gate catégories crm+billing+support, pageKey `audit_automatisations` conservée. (4) **Renommages** : sidebar/PAGE_LABELS/tool-mapping-settings (« Alignement », + « Audit qualité » pour audit_donnees)/sync-blocks/recommandations ; agent `automatisations` → « Agent Alignement » (expertise réécrite relais/SLA, tools + billing, persona « Agent alignement », script audio réécrit — vidéo à régénérer). **Manuel** : migration `20260725000001` + resync full HubSpot pour l'historique des dates. |
| 2026-07-25 | Funnel tables — période v2 | Option « description » + plage personnalisée | Champ Période du funnel (création + édition, composant partagé `PeriodField`) : (1) option **« Déjà précisée dans la description »** (parcours agent uniquement) — l'agent extrait les dates absolues du texte du KPI (BUILD_TOOL `date_from`/`date_to` + date du jour dans le system prompt) et la route les fige en plage `custom:from..to` ; sans dates extraites, sentinelle `description` = aucun re-filtre à l'ouverture. (2) option **« Plage de dates personnalisée »** : 2 inputs date (du/au), validation début ≤ fin (CTA bloqué sinon), sérialisée `custom:YYYY-MM-DD..YYYY-MM-DD` dans `period_preset` (pas de migration). `periods.ts` : `parseStoredPeriod`/`sanitizeStoredPeriod`/`serializeCustomPeriod`/`storedPeriodLabel` ; `cleanPeriod` (routes) délègue à `sanitizeStoredPeriod`. `DataTableCard` ouvre la table sur la plage figée (label « 12 janv. 2026 → 03 mars 2026 ») ou sans filtre si `description`. tsc + eslint OK. |
...
| 2026-05-03 | Slack relay (workaround) | Endpoint `/api/admin/slack-relay` (Vercel) qui forwarde vers webhook Slack | Découverte : sandbox CCR Anthropic bloque les outbound vers `hooks.slack.com` directement. Tentative de workaround via relay Vercel : SLACK_RELAY_SECRET + SLACK_WEBHOOK_URL en env, vérif HMAC headers. **Échec** : sandbox bloque aussi `revold.ai` (allowlist très restrictive) |
...
| 2026-07-13 | Ops | `ANTHROPIC_API_KEY` ajoutée en env Vercel (prod/preview/dev) + crédits Anthropic | Diagnostic « rien ne passe » = clé absente puis solde crédits à zéro (la chaîne agent fonctionnait de bout en bout). **Découverte : le push git auto-déploie désormais (intégration Git Vercel active) et promeut sur revold.ai — `vercel --prod` CLI en doublon crée un déploiement non promu.** Mémoire de déploiement corrigée. Clé API exposée en clair dans le chat → à rotater. |
