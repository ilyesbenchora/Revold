...
> **Statut global** : Phase 6 finalisée — Phase 7 amorcée — Phase 8 majoritairement faite : 8.1 OAuth HubSpot multi-tenant ✅, 8.2 Stripe billing ✅ (config Stripe Dashboard à finaliser côté user), 8.3 sync engine robuste ✅, 8.4 notifications email + Slack/Teams ✅, 8.5 RBAC équipe + invitations + audit log ✅, 8.9 sécurité publique + DPA + EU ✅. Restent : 8.6 perf scaling > 50k contacts, 8.7 activity capture (Aircall/Chrome), 8.8 tests Vitest + Sentry, 8.10 onboarding wizard, **8.11 marketplace listings (HubSpot App + Stripe Partner)** = canal d'acquisition organique vs Clari.
