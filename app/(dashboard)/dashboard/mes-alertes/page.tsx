...
export default async function MesAlertesPage() {
  ...
  const COLS: string = "id, title, description, impact, category, status, threshold, unit_mode, created_at, date_from, date_to, notification_channels, forecast_type, agg_spec, recon_spec, scope, cross_sources, owner_name";
  let res = await supabase.from("alerts").select(COLS).eq("organization_id", orgId).order("created_at", { ascending: false }).limit(200);
  if (res.error && /(agg_spec|recon_spec|scope|cross_sources|owner_name)/.test(res.error.message)) {
    const legacy = COLS.replace(", agg_spec", "").replace(", recon_spec", "").replace(", scope", "").replace(", cross_sources", "").replace(", owner_name", "");
  ...
  const rdy = (a: AlertRow) => isKpiDataReady(readySet, a.forecast_type, a.agg_spec, a.recon_spec?.recipe);
