...
export default async function MarketingCoachingPage() {
  ...
  const [ctx, { dismissedKeys }, { workflows }, reportCoachings] = await Promise.all([
    buildContext(supabase, orgId),
    fetchDismissals(supabase, orgId),
    fetchWorkflows(token),
    fetchReportCoachings(supabase, orgId, "marketing"),
  ]);
  ...
  return (
    <div className="space-y-6">
      <ReportCoachingsSection coachings={reportCoachings} category="marketing" />
      {allEmpty ? (
