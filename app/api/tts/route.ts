...
export async function POST(request: Request) {
  ...
  const persona = body.agentKey ? PERSONA_SCRIPTS[body.agentKey] : undefined;
  const voiceId = persona?.elevenVoiceId ?? FALLBACK_VOICE;
  ...
  const res = await fetch(
    `https://api.elevenlabs.io/v1/text-to-speech/${voiceId}?output_format=mp3_44100_96`,
    {
      ...
      body: JSON.stringify({
        ...
        // Turbo v2.5 : FR natif, latence faible — adapté au conversationnel.
