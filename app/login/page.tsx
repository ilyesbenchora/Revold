  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-slate-950 px-4">
      <section className="w-full max-w-md rounded-2xl border border-slate-800 bg-slate-900 p-8 shadow-2xl">
        ...
        {/* ── Connexion SSO entreprise (SAML 2.0 — Entra ID, Okta, Google WS) ── */}
        {ssoMode && (
          <>
            <form action={ssoAction} className="mt-8 space-y-4">
              <div>
                <label htmlFor="sso_email" className="mb-1 block text-sm font-medium text-slate-300">
                  Email professionnel
                </label>
                <input
                  id="sso_email"
                ...
                <p className="mt-1.5 text-[11px] leading-relaxed text-slate-500">
                  Ton domaine détermine l&apos;annuaire de ton entreprise (Microsoft Entra ID, Okta, Google
                  Workspace…) : tu es redirigé vers sa page de connexion — aucun mot de passe Revold.
                </p>
              </div>
              <SubmitButton
                className="w-full rounded-lg bg-accent px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-indigo-500"
                pendingLabel="Redirection vers ton annuaire…"
                spinnerClass="border-white/40 border-t-white"
              >
        ...
        {!otpMode && !entrepriseMode && !ssoMode && (
        ...
        {/* Lien SSO entreprise — sous le bouton Google, discret mais visible des DSI. */}
        {!otpMode && !entrepriseMode && !ssoMode && (
          <p className="mt-3 text-center">
            <a href="/login?mode=sso" className="text-xs font-medium text-slate-400 underline-offset-2 transition hover:text-slate-200 hover:underline">
              🔐 Connexion SSO entreprise (SAML — Microsoft Entra ID, Okta…)
            </a>
          </p>
        )}
        ...
        {!otpMode && !entrepriseMode && !passwordMode && !ssoMode && (
          <>
