...
/**
 * Onglets de la page Ventes — PERSONNALISABLES (table page_nav) : les onglets
 * standard (Cycle de ventes, Transactions à risque, Transactions expirées)
 * peuvent être renommés, et des pages custom ajoutées (rendues sur /p/[slug],
 * avec leurs propres tuiles KPI et tableaux configurables).
 */
export function VentesTabs() {
  const pathname = usePathname();
  const router = useRouter();
  const [tabs, setTabs] = useState<PageNavItem[]>(VENTES_NAV.defaults);
  ...
  useEffect(() => {
    let cancelled = false;
    fetch(`/api/page-nav?nav_key=${VENTES_NAV.navKey}`)
      .then((r) => (r.ok ? r.json() : null))
      .then((d) => {
        if (!cancelled && d) setTabs(mergeNavItems(VENTES_NAV, d.items as PageNavItem[]));
      })
      .catch(() => {});
    return () => {
      cancelled = true;
  ...
  async function save() {
    ...
    try {
      const res = await fetch("/api/page-nav", {
        ...
        body: JSON.stringify({ nav_key: VENTES_NAV.navKey, items }),
      });
      ...
      setTabs(mergeNavItems(VENTES_NAV, (d.items as PageNavItem[]) ?? items));
      setEditing(false);
      router.refresh();
    } catch {
  ...
  return (
    <div className="border-b border-card-border">
      <div className="flex items-center gap-1">
        {tabs.map((t) => {
          const href = navItemHref(VENTES_NAV, t);
