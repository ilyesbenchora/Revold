...
type HierarchySort = { col: "parent" | "child"; dir: "asc" | "desc" } | null;

/** En-tête de colonne triable (Parent / Enfant) : A→Z → Z→A → ordre de détection. */
function SortableHeader({
  ...
  label: string;
  sort: HierarchySort;
  onSort: (next: HierarchySort) => void;
}) {
  const active = sort?.col === col;
  return (
    <button
      type="button"
      onClick={() => onSort(active ? (sort!.dir === "asc" ? { col, dir: "desc" } : null) : { col, dir: "asc" })}
      title={active ? (sort!.dir === "asc" ? "Trier Z→A" : "Revenir à l'ordre de détection") : "Trier A→Z"}
...
export function HierarchyConsole({
  ...
  // Tri alphabétique des colonnes Parent / Enfant (vue Table) — clic sur
  // l'en-tête : A→Z, re-clic : Z→A, re-clic : retour à l'ordre de détection.
  const [sort, setSort] = useState<{ col: "parent" | "child"; dir: "asc" | "desc" } | null>(null);
  ...
  // Noms AFFICHÉS d'une ligne (sens éventuellement inversé) — base du tri :
  // on trie ce que l'utilisateur lit, pas le payload brut.
  const displayNames = (a: ActionRow) => {
    ...
    return { parent: isSwapped ? rawChild : rawParent, child: isSwapped ? rawParent : rawChild };
  };
  const sortedVisible =
    visible && sort
      ? [...visible].sort((a, b) => {
          const cmp = displayNames(a)[sort.col].localeCompare(displayNames(b)[sort.col], "fr", { sensitivity: "base" });
                  ...
                  <table className="w-full text-left text-xs">
                    <thead>
                      <tr className="border-b border-slate-200 bg-slate-50/70 text-[11px] uppercase tracking-wide text-slate-500">
                        ...
                        {/* Parent / Enfant : tri alphabétique au clic — A→Z, Z→A, puis ordre de détection. */}
                        <th className="px-2.5 py-2 font-semibold">
                          <SortableHeader col="parent" label="Parent" sort={sort} onSort={(s) => { setSort(s); setPage(0); }} />
                        </th>
                        <th className="px-2.5 py-2 font-semibold">SIREN · SIRET (parent)</th>
                        <th className="px-2.5 py-2 font-semibold">
                          <SortableHeader col="child" label="Enfant" sort={sort} onSort={(s) => { setSort(s); setPage(0); }} />
                        </th>
