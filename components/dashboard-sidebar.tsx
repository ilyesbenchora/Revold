...
const auditChildren: LeafLink[] = [
  ...
  {
    // Page préparée : dédiée au phoning (Aircall, Ringover…), s'active à la
    // connexion d'un outil d'appels.
...
const dashboardChildren: LeafLink[] = [
  ...
  // « Mes alertes » vit dans le menu dédié Alertes — pas de doublon ici.
];

const alertesChildren: LeafLink[] = [
  {
  ...
  {
    href: "/dashboard/mes-alertes/calendrier",
    label: "Calendrier",
    icon: (
...
const integrationsChildren: LeafLink[] = [
  ...
  // Outil hors catalogue (ERP maison, logiciel métier) : endpoints déclarés
  // par le client, rapprochement par l'ID de rapprochement.
  {
    href: "/dashboard/integration/sur-mesure",
...
const sidebarLinks: SidebarItem[] = [
  ...
  // Enrichissement = ACTIONS à forte valeur sur la donnée entreprises :
  // groupe à deux pages — enrichissement officiel (Sirene/INPI) et
  // hiérarchies de comptes parent/enfant (groupes multi-sociétés).
  {
    id: "enrichissement",
    ...
    children: [
      ...
      },
      {
        ...
        label: "Hiérarchie comptes",
        icon: (
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="9" y="3" width="6" height="5" rx="1" /><rect x="3" y="16" width="6" height="5" rx="1" /><rect x="15" y="16" width="6" height="5" rx="1" /><path d="M12 8v3" /><path d="M6 16v-3h12v3" /></svg>
        ),
      },
    ],
  },
