...
import { PAGE_LABELS } from "@/lib/reports/data-table-presets";
        ...
        <div className="absolute right-0 z-20 mt-1 w-64 rounded-xl border border-card-border bg-card p-2 shadow-xl">
          ...
          {Object.entries(PAGE_LABELS).map(([key, label]) => (
            <button
              ...
              type="button"
              disabled={busy}
              onClick={() => void assign(key)}
              className="block w-full rounded-lg px-2 py-1.5 text-left text-xs text-slate-700 transition hover:bg-slate-50 disabled:opacity-50"
            >
              {label}
            </button>
          ))}
